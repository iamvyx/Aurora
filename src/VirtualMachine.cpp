...
void VirtualMachine::Destroy() { delete this; }
...
void VirtualMachine::Parse(std::string path)
{
    ...
    char  *bytecode     = luau_compile(sourceCode.c_str(), sourceCode.length(),
    ...
    try {
        ...
        if (m_luau == nullptr) {
            throw std::runtime_error(
                (std::stringstream("Virtual Machine Error: ")
                 << To<std::string>(-1))
                    .str());
        }
